...
public class MoreLikeThisHelper
{
    ...
    public MLTResult getMoreLikeTheseFromDocs(DocIterator iterator, int start, int rows, List<Query> filters, int flags, Sort lsort, Query mainQuery) throws IOException, SyntaxError
    {
        ...
        if(mainQuery != null){
            ...
            finalQuery.add(realMLTQuery, BooleanClause.Occur.SHOULD);
        }
        else{
            finalQuery = realMLTQuery;
        }
        mltResult.setFinalQuery(finalQuery);
    ...
    public MLTResult getMoreLikeThisFromContentSteam(Reader reader, int start, int rows, List<Query> filters, int flags, Sort lsort, Query mainQuery) throws IOException, SyntaxError
    {
        ...
        if(mainQuery != null){
            ...
            tmpQuery .add(boostedMLTQuery, BooleanClause.Occur.SHOULD);
            finalQuery = tmpQuery;
        }
        else{
            finalQuery = boostedMLTQuery;
        }
        mltResult.setFinalQuery(finalQuery);
