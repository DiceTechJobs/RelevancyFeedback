...
To manually compile it for a later version, use maven to compile the plugins using the pom.xml file, and update the versions of the jars in that file. Then fix any build errors. Later versions of solr (6.x) use query builders to build the boolean queries, which is likely what needs to be fixed to support 6.x.
